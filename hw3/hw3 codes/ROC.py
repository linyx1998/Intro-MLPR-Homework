...
def draw_min_error(name):
    samples = read_data_2d(name)[0]
    labels = read_data_2d(name)[1]

    x, y, bg, tg, te, me = estimate_error(samples, labels)
...
def estimate_roc(samples, labels):
    ...
    gamma = np.concatenate((gamma_1, gamma_2, gamma_3, np.arange(1.47, 1.53, 0.001)))
    gamma.sort()
...
def estimate_error(samples, labels):
    gamma_1 = np.append(np.arange(0, 0.2, 0.01), np.arange(0.2, 2, 0.1))
    gamma_2 = np.append(np.arange(2, 20, 1), np.arange(20, 200, 10))
